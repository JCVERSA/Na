...
module.exports = {
  ...
  async execute(sock, msg, args, extra) {
    try {
      ...
      const aiBlock = section('🤖', 'AI', [
        c(p,'ai','Nebula AI (multi-model)'),
        c(p,'gem','Gemini 2.0 Flash'),
        c(p,'grok','Groq — LLaMA 3.3 (ultra fast)'),
        c(p,'imagine','Generate an AI image'),
        c(p,'define','Define a word or concept'),
        c(p,'fact','Random surprising fact'),
        c(p,'summarize','AI summary of a long text'),
      ]);

      const msg2 = [adminBlock, aiBlock].join('\n\n');
      ...
      // MSG 4 — ECONOMY + NEONPARTY
      const ecoBlock = section('💰', 'ECONOMY & LEVELS', [
        c(p,'balance','Full profile: coins, rank, streak [@user]'),
        c(p,'daily','Daily reward + streak bonus 🔥'),
        c(p,'work','Work for coins (30min cooldown)'),
        c(p,'pay','Send coins to @user'),
        c(p,'collect','Passive income (every 2h)'),
        c(p,'streak','Streak details [@user]'),
        c(p,'levels','All titles and your progression'),
        '',
        '🏦 *BANK & FINANCE*',
        c(p,'bank','Bank (deposit/withdraw — safe from rob)'),
        c(p,'loan','Borrow coins (+25% interest, 48h)'),
        c(p,'invest','Invest (-30% to +80%, returns in 24h)'),
        '',
        '🎲 *CASINO*',
        c(p,'slots','Slot machine 🎰 (5min)'),
        c(p,'gamble','50/50 bet — double or nothing'),
        c(p,'wheel','Wheel of fortune 🎡 (10min)'),
        c(p,'dice','Dice bet (high/low/exact) 🎲'),
        c(p,'blackjack','Blackjack vs bot 🃏'),
        c(p,'scratch','Scratch card 🎫 (1h)'),
        c(p,'lottery','Community lottery 🎰'),
        '',
        '⚔️ *SOCIAL & DUELS*',
        c(p,'duel','1v1 coin stake duel'),
        c(p,'coinflip','Coin flip vs someone 🪙'),
        c(p,'heist','Group heist 2-5 players 🔫'),
        c(p,'bounty','Put/claim a bounty'),
        c(p,'gift','Gift an item 🎁'),
        '',
        '🌿 *ACTIVITIES*',
        c(p,'fish','Fishing 🎣 (10min)'),
        c(p,'mine','Mining ⛏️ (15min)'),
        c(p,'hunt','Hunting 🏹 (20min)'),
        c(p,'beg','Beg 🙏'),
        '',
        '🦹 *CRIME*',
        c(p,'rob','Steal from @user (risky!) 🔫'),
        c(p,'crime','Random crime (1h)'),
        '',
        '🛒 *SHOP*',
        c(p,'shop'), c(p,'buy'), c(p,'inventory'),
      ]);

      const neonBlock = section('⚔️', 'NEONPARTY — RPG', [
        c(p,'startneonparty','Join the RPG game'),
        c(p,'gprofile','Game profile [@player]'),
        c(p,'kill','Attack a player'),
        c(p,'revive','Revive [@player]'),
        c(p,'heal','Use a healing potion'),
        c(p,'equip','Equip weapon/armor/power'),
        c(p,'setgamename','Change game username'),
        '',
        '💍 *FAMILY*',
        c(p,'marry','Marriage proposal'),
        c(p,'divorce','Divorce'),
        c(p,'adopt','Adopt a player'),
        c(p,'family','Family tree'),
        '',
        '🏆 *RANKINGS*',
        c(p,'richlist','Top 10 richest'),
        c(p,'topxp','Top 10 by level'),
        c(p,'pvprank','PVP leaderboard'),
      ]);

      const msg4 = [ecoBlock, neonBlock].join('\n\n');

      // MSG 5 — FUN + ANIME + TOOLS + OWNER
      ...
      const ownerBlock = section('👑', 'OWNER ONLY', [
        ...
        '',
        '💰 *ECONOMY ADMIN*',
        c(p,'givecoins','Give coins to @player <amount>'),
        c(p,'givelevel','Give levels to @player <amount>'),
        c(p,'giverank','Set NeonParty rank for @player <1-20>'),
        '',
        '🎮 *REMOTE CONTROL*',
      ...
      const msg5 = [funBlock, animeBlock, statsBlock, utilityBlock, ownerBlock].join('\n\n') + footer;
